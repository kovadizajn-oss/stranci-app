...
export async function GET(request: Request) {
  ...
  const [{ data: docs, error }, { data: obaveze }] = await Promise.all([
    supabase
      .from('documents')
    ...
    supabase
      .from('obaveze')
      .select('id, naziv, rok, employees(ime, prezime)')
      .lte('rok', in7Str)
      .gte('rok', todayStr)
  ...
  if ((!docs || docs.length === 0) && (!obaveze || obaveze.length === 0)) {
  ...
  const obavezeRows = (obaveze || []).map((o: any) => {
    const emp = o.employees
    const days = daysUntil(o.rok)
    const urgency = days <= 2 ? '#DC2626' : days <= 5 ? '#CA8A04' : '#475569'
    return `
      <tr>
        <td style="padding:10px 14px;border-bottom:1px solid #F1F5F9;font-size:14px;color:#1E293B;">
          ${emp ? `${emp.ime} ${emp.prezime}` : '—'}
        ...
        <td style="padding:10px 14px;border-bottom:1px solid #F1F5F9;font-size:14px;">
          <span style="color:${urgency};font-weight:600;">${formatDate(o.rok)}</span>
        </td>
        <td style="padding:10px 14px;border-bottom:1px solid #F1F5F9;font-size:14px;color:${urgency};font-weight:600;">
          ${days === 0 ? 'Danas!' : days === 1 ? 'Sutra' : `Za ${days} dana`}
        </td>
  ...
  const docsSection = docs && docs.length > 0 ? `
  ...
  const obavezeSection = obaveze && obaveze.length > 0 ? `
    <div style="padding:20px 24px 8px;border-top:2px solid #F1F5F9;">
      <p style="margin:0 0 12px;font-size:14px;color:#475569;">
        Obaveze s rokom u sljedećih <strong>7 dana</strong>:
      </p>
      <table style="width:100%;border-collapse:collapse;background:white;">
        <thead>
          <tr style="background:#F8FAFC;">
            <th style="padding:10px 14px;text-align:left;font-size:12px;color:#94A3B8;font-weight:600;border-bottom:2px solid #E2E8F0;">RADNIK</th>
            <th style="padding:10px 14px;text-align:left;font-size:12px;color:#94A3B8;font-weight:600;border-bottom:2px solid #E2E8F0;">OBAVEZA</th>
            <th style="padding:10px 14px;text-align:left;font-size:12px;color:#94A3B8;font-weight:600;border-bottom:2px solid #E2E8F0;">ROK</th>
            <th style="padding:10px 14px;text-align:left;font-size:12px;color:#94A3B8;font-weight:600;border-bottom:2px solid #E2E8F0;">ZA</th>
          </tr>
        </thead>
        <tbody>${obavezeRows}</tbody>
      </table>
    </div>` : ''

  const totalItems = (docs?.length || 0) + (obaveze?.length || 0)
    ...
    <div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#F8FAFC;padding:24px;">
      <div style="background:white;border-radius:12px;overflow:hidden;border:1px solid #E2E8F0;">
        <div style="background:#2563EB;padding:20px 24px;">
          <h1 style="margin:0;color:white;font-size:18px;font-weight:600;">Kvantus Admin: Dnevni pregled</h1>
  ...
  await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: ['info@kvantus.com'],
      subject: `Kvantus Admin — dnevni pregled (${totalItems} stavki)`,
      html,
    }),
  })

  return NextResponse.json({ sent: true, docs: docs?.length || 0, obaveze: obaveze?.length || 0 })
}
